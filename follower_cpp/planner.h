...
class planner
{
    ...
    std::vector<std::vector<int>> count_occupations; //我加的
...
public:
    ...
    grid(_grid), use_static_cost(_use_static_cost), use_dynamic_cost(_use_dynamic_cost), reset_dynamic_cost(_reset_dynamic_cost)
    {
        ...
        count_occupations = std::vector<std::vector<int>>(grid.size(), std::vector<int>(grid.front().size(), 0));//我加的
    ...
    void update_occupations(py::array_t<double> array, std::pair<int, int> cur_pos, std::pair<int, int> cur_goal, bool isReward_on_goal)
    {
        cur_goal = {cur_goal.first + abs_offset.first, cur_goal.second + abs_offset.second};
        float decay_rate = 0.05f;
        if(reset_dynamic_cost)
            if(isReward_on_goal == 1)
                num_occupations = std::vector<std::vector<float>>(grid.size(), std::vector<float>(grid.front().size(), 0));
        py::buffer_info buf = array.request();
        std::list<std::pair<int, int>> occupied_cells;
        std::list<std::pair<int, int>> empty_occupied_cells;
        ...
        for (size_t i = 0; i < num_occupations.size(); i++) {
            for (size_t j = 0; j < num_occupations[0].size(); j++) {
                num_occupations[i][j] *= (1.0f - decay_rate);
            }
        }
