...
def activation_func(cfg: EncoderConfig) -> nn.Module:#返回激活函数
    """
...
class ResBlock(nn.Module):#这个类定义了空间编码器的一个残差块 Residual Block
    """
...
class ResnetEncoder(Encoder):#空间编码器的网络实现
    """
    ...
    def __init__(self, cfg: Config, obs_space: ObsSpace):
        ...
        input_ch = obs_space['obs'].shape[0]#获取输入颜色通道
        resnet_conf = [[self.encoder_cfg.num_filters, self.encoder_cfg.num_res_blocks]]
        curr_input_channels = input_ch
        layers = []
        for out_channels, res_blocks in resnet_conf:#定义网络
