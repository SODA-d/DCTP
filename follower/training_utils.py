...
def run(config=None):
    register_custom_model()#注册了一个空间编码器（由resnet实现）
    ...
    register_custom_components(env_name)#注册环境
    ...
    flat_config, runner = make_runner(create_sf_config(exp))#传入参数，生成保存模型文件文件夹
